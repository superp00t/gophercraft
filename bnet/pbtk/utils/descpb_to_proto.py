...
# from bgs import MethodOptions
from utils.method_options_pb2 import method_options
from utils.service_options_pb2 import service_options
    ...
    if is_msg:
        ...
        for service in desc.service:
            ...
            for method in service.method:
                out2 += 'rpc %s(%s%s) returns (%s%s)' % (method.name,
                ...
                print("OPTIONS::" + method.name);
                if method.options.HasExtension(method_options):
                    method_id = method.options.Extensions[method_options].id;
                    out2 += ' {\n\t';
                    out2 += 'option (method_options).id = ' + str(method_id) + ';\n}'
                out2 += ';\n'
                print(method.options.HasExtension(method_options))
                # pprint.pprint(method.GetOptions());
...
def wrap_block(type_, value, desc=None, name=None):
    ...
    if desc:
        for (option, optval) in desc.options.ListFields():
            if option.name == "service_options" or option.name == "sdk_service_options":
                for (field, val) in optval.ListFields():
                    value = 'option (' + option.name + ').' + field.name + ' = ' + fmt_value(val, desc=field) + ';\n' + value
            else:
